...
for n = 1:Np
    ...
    rho(mod(i(n)+1,Ng)+1,j(n)+1,k(n)+1) = ...
        rho(mod(i(n)+1,Ng)+1,j(n)+1,k(n)+1) + mp*tx(n)*ty(n)*tz(n);
    
    rho(i(n)+1,mod(j(n)+1,Ng)+1,k(n)+1) = ...
        rho(i(n)+1,mod(j(n)+1,Ng)+1,k(n)+1) + mp*tx(n)*ty(n)*tz(n);
    
    rho(i(n)+1,j(n)+1,mod(k(n)+1,Ng)+1) = ...
        rho(i(n)+1,j(n)+1,mod(k(n)+1,Ng)+1) + mp*tx(n)*ty(n)*tz(n);
    
    rho(mod(i(n)+1,Ng)+1,mod(j(n)+1,Ng)+1,k(n)+1) = ...
        rho(mod(i(n)+1,Ng)+1,mod(j(n)+1,Ng)+1,k(n)+1) + mp*tx(n)*ty(n)*tz(n);
    
    rho(mod(i(n)+1,Ng)+1,j(n)+1,mod(k(n)+1,Ng)+1) = ...
        rho(mod(i(n)+1,Ng)+1,j(n)+1,mod(k(n)+1,Ng)+1) + mp*tx(n)*ty(n)*tz(n);
    
    rho(mod(i(n)+1,Ng)+1,mod(j(n)+1,Ng)+1,k(n)+1) = ...
        rho(mod(i(n)+1,Ng)+1,mod(j(n)+1,Ng)+1,k(n)+1) + mp*tx(n)*ty(n)*tz(n);
    
    rho(mod(i(n)+1,Ng)+1,mod(j(n)+1,Ng)+1,mod(k(n)+1,Ng)+1) = ...
        rho(mod(i(n)+1,Ng)+1,mod(j(n)+1,Ng)+1,mod(k(n)+1,Ng)+1) + mp*tx(n)*ty(n)*tz(n);
